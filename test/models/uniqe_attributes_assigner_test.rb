...
class UniqueAttributesAssignerTest < ActiveSupport::TestCase
  # Setting up two link objects with identical attributes. First is saved to db on create.
  def setup
  ...
  test 'ensure unique attributes before save' do
    assert_difference('Link.count') do
      @link2 = UniqueAttributesAssigner.ensure_unique(@link2)
      @link2.save
    end
  end
